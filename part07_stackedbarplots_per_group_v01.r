...
wd00 <- "present_working_directory"
 setwd(wd00)
#inp_fn1 <- paste("part03_DADA2_nochim.table.",libnm,".txt",sep="")
inp_fn1 <- "DADA2_nochim.table"
inp_fn1 <- "DADA2_nochim.table_repl1.txt"
#inp_fn1 <- "DADA2_nochim.table_120lines.txt"
...
inp_fn2 <- "classified.txt"
#inp_fn2 <- "classified_120lines.txt"
...
#inp_fn4 <- "part07_list_of_plausible_species.txt"
...
df_blast_filt_01 <- read.table(file=inp_fn2,sep='\t',header=T, 
...
blast_unfilt_kingdom <- df_blast_unfilt_01[,8] #kingdom
blast_unfilt_phylum <- df_blast_unfilt_01[,9] #phylum
blast_unfilt_class <- df_blast_unfilt_01[,10] #class
blast_unfilt_order <- df_blast_unfilt_01[,11] #order
blast_unfilt_fam <- df_blast_unfilt_01[,12] #family
blast_unfilt_genus <- df_blast_unfilt_01[,13] #genus
blast_unfilt_genus_species <- df_blast_unfilt_01[,14] #genus_species

blast_filt_seqID <- df_blast_filt_01$qseqid
blast_filt_kingdom <- df_blast_filt_01[,8] #kingdom
blast_filt_phylum <- df_blast_filt_01[,9] #phylum
blast_filt_class <- df_blast_filt_01[,10] #class
blast_filt_order <- df_blast_filt_01[,11] #order
blast_filt_fam <- df_blast_filt_01[,12] #family
blast_filt_genus <- df_blast_filt_01[,13] #genus
blast_filt_genus_species <- df_blast_filt_01[,14] #genus_species

#convert data frame from a "wide" format to a "long" format
df_nochim01_melt <- reshape2::melt(tbl_nochim01, id = c("X"))
...
df_nochim01_melt$blast_unfilt_kingdom <- df_blast_unfilt_01[,8][match(df_nochim01_melt$SeqNo,df_blast_unfilt_01$qseqid)]
df_nochim01_melt$blast_unfilt_phylum <- df_blast_unfilt_01[,9][match(df_nochim01_melt$SeqNo,df_blast_unfilt_01$qseqid)]
df_nochim01_melt$blast_unfilt_class <- df_blast_unfilt_01[,10][match(df_nochim01_melt$SeqNo,df_blast_unfilt_01$qseqid)]
df_nochim01_melt$blast_unfilt_order <- df_blast_unfilt_01[,11][match(df_nochim01_melt$SeqNo,df_blast_unfilt_01$qseqid)]
df_nochim01_melt$blast_unfilt_fam <- df_blast_unfilt_01[,12][match(df_nochim01_melt$SeqNo,df_blast_unfilt_01$qseqid)]
df_nochim01_melt$blast_unfilt_genus <- df_blast_unfilt_01[,13][match(df_nochim01_melt$SeqNo,df_blast_unfilt_01$qseqid)]
df_nochim01_melt$blast_unfilt_genus_species <- df_blast_unfilt_01[,14][match(df_nochim01_melt$SeqNo,df_blast_unfilt_01$qseqid)]
#match between data frames
df_nochim01_melt$blast_filt_kingdom <- df_blast_filt_01[,8][match(df_nochim01_melt$SeqNo,df_blast_filt_01$qseqid)]
df_nochim01_melt$blast_filt_phylum <- df_blast_filt_01[,9][match(df_nochim01_melt$SeqNo,df_blast_filt_01$qseqid)]
df_nochim01_melt$blast_filt_class <- df_blast_filt_01[,10][match(df_nochim01_melt$SeqNo,df_blast_filt_01$qseqid)]
df_nochim01_melt$blast_filt_order <- df_blast_filt_01[,11][match(df_nochim01_melt$SeqNo,df_blast_filt_01$qseqid)]
df_nochim01_melt$blast_filt_fam <- df_blast_filt_01[,12][match(df_nochim01_melt$SeqNo,df_blast_filt_01$qseqid)]
df_nochim01_melt$blast_filt_genus <- df_blast_filt_01[,13][match(df_nochim01_melt$SeqNo,df_blast_filt_01$qseqid)]
df_nochim01_melt$blast_filt_genus_species <- df_blast_filt_01[,14][match(df_nochim01_melt$SeqNo,df_blast_filt_01$qseqid)]
...
df_ncm02 <- df_nochim01_melt[
  with(df_nochim01_melt, order(blast_unfilt_kingdom,
                               blast_unfilt_phylum,
                                blast_unfilt_class,
                                blast_unfilt_order,
                                blast_unfilt_fam,
...
df_ncm03_unfilt_tspc <- df_ncm02[ which(df_ncm02$blast_unfilt_kingdom=="Metazoa" 
                                        | df_ncm02$blast_unfilt_kingdom=="NA"), ]
df_ncm03_unfilt_tspc <- df_ncm02[ which(df_ncm02$blast_unfilt_kingdom=="Metazoa" 
                                        | df_ncm02$blast_unfilt_class=="NA"), ]
...
df_ncm03_unfilt_not_tspc <- df_ncm02[ which(!df_ncm02$blast_unfilt_kingdom=="Metazoa"
                                            & !df_ncm02$blast_unfilt_kingdom=="NA"), ]
#uncomment the next two lines if you want to check the unique elements in the new data frames
#unique(df_ncm02$blast_unfilt_class)
#unique(df_ncm03_unfilt_not_tspc$blast_unfilt_class)
df_ncm03_filt_tspc <- df_ncm02[ which(df_ncm02$blast_unfilt_kingdom=="Metazoa"
                                      | df_ncm02$blast_unfilt_kingdom=="NA"), ]
#uncomment the next two lines if you want to check the unique elements in the new data frames
#unique(df_ncm02$blast_filt_class)
#unique(df_ncm03_filt_tspc$blast_filt_class)
df_ncm03_filt_not_tspc <- df_ncm02[ which(!df_ncm02$blast_unfilt_kingdom=="Metazoa" 
                                    & !df_ncm02$blast_unfilt_kingdom=="NA"), ]
#uncomment the next two lines if you want to check the unique elements in the new data frames
#unique(df_ncm02$blast_filt_class)
...
if (!identical(lst_unfbgenspc_mck, character(0)) ) {
  print("0") 
...
# check if data frame exits
if (exists("df_cfsmsp01"))
  {
...
# get number of elements in the class-order-fam-gen-spc column
lubu <- length(df_ncm03$blast_unfilt_class_ord_fam_gen_spc)
# get number of elements for the colors
lu <- length(uufgsc3)
# check if these do not match
if (lubu != lu)
  #if they do not match
{
  #subtract the two values to get the needed number of elements 
  nmis <- lubu-lu
  # make a sequence of fractions
  zv <- 1/seq(1,nmis)
  # make a color range 
  f <- colorRamp(c("gray80", "gray30"))
  # make a color range based on fractions
  hexcol04 <- (colors <- rgb(f(zv)/255))
  # add the extra colors to the color range
  uufgsc3 <- c(uufgsc3,hexcol04)
  }
...
df_ncm05 <- df_ncm03[ which(df_ncm03$blast_unfilt_kingdom=="Metazoa" 
                            | df_ncm03$blast_unfilt_kingdom=="NA"), ]
...
# check if the data frame is not empty. If it is not empty, then make a plot
if (!plyr::empty(df_ncm05))
  {
#create 100% stacked bar plot with seqN on the x-axis
...
# is it is empty,then make an empty plot
} else {Stacked_bar_plot6 <- plot.new()}
...
#if the data frame is empty, then make an empty plot
} else { Stacked_bar_plot5 <- plot.new()}
...
figure <- ggpubr::ggarrange(Stacked_bar_plot3,
                    Stacked_bar_plot4,
                    Stacked_bar_plot5,
                    Stacked_bar_plot6,
                    labels = c("A", "B", "C", "D"),
...
if (!plyr::empty(df_spa_fexno_02))
{
  # it it is not empty, then use the the function above  
...
#           row.names = FALSE)
# 
# # end if test to check whether the data frame is not empty
}
# #____________________________________________________________________________________
